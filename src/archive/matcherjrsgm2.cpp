...
void MatcherJRSGM::init(void) {
    ...
    //const std::string sConfigFile =  QCoreApplication::applicationDirPath().toStdString() + "/param/jrsgm.param";
    //qDebug() << sParamFile.c_str();

    //ReadIniFile(oParams,sParamFile);

    //saveParameters();

    createMatcher();

    //writeConfig(sConfigFile,oParams);
}

void MatcherJRSGM::createMatcher()
{
    ...
    try
    {
        ...
        matcher_handle = CreateMatchStereoHandle(oParams);
...
SSGMParameters MatcherJRSGM::initSGMParameters() {
    ...
    oSGMParams.fP2_SW_NE = 0.800000f;
    ...
    oSGMParams.fP2_SE_NW = 0.800000f;
...
void MatcherJRSGM::initParameters(int nPyramids) {
    SSGMParameters oSGMParams = initSGMParameters();

    oParams.bHasNodata = true;
    oParams.fNodataValue = 0;
    ...
    oParams.fTopPredictionShift = -22;
    //params.nAdaptivePrefilter = 0;
    //params.nAdaptivePrefilterSize = 3;
    oParams.nNumberOfMorphologicalIter = 1;
    oParams.nDebugLevel = 0;
    oParams.strDebugOutput = "";
    //params.oProcessAffinity = std::set<int>();
    ...
    for (auto &pyramid : oParams.oPyramidParams) {
        ...
        pyramid.iSpeckleMaxSize = 100;  //"Disparity Speckle Filter Max Region Size"
        ...
        pyramid.oMetricParams.bUseProximityWeight = 1;
    ...
    SMatchingParameters oSubPix;
    // oFinalSubPixelParameters
    oSubPix.bCompute = 1;
    oSubPix.nMetric = 8;
    oSubPix.nMaximumNumberOfDisparities = 31;
    oSubPix.fDisparityStepSize = 1.000000f;
...
void MatcherJRSGM::setDisparityRange(int n) {
    if (n % 2) {
        ...
        oParams.oFinalSubPixelParameters.nMaximumNumberOfDisparities = disparity_range;
...
void MatcherJRSGM::forwardMatch() {
    try {
        ...
        MatchStereo(matcher_handle,*left,*right,cv::Mat(),cv::Mat(),oDisparity,sgm_log,e_logError);
